...
public class Tree {
	...
	int maxDist = 25;
	/**
	 * give it where the tree meets ground
	 * @param head
	 */
	public Tree(Vector3f seed) {
		this.head = seed;
		this.seed = seed;
		blocks.add(head);
		tree();
	 ...
	 */
	public void tree(){
		int height = rng.nextInt(5)+5;
		maxDist = height;
		for(int i = 0; i<height; i++){
			blocks.add(head.add(down.scale(i)));
		...
		for(int i = 0; i<blocks.size(); i++){
			blocks.set(i, blocks.get(i).add(new Vector3f(0,0,height)));
		}

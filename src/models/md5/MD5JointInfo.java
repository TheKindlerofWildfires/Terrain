...
public class MD5JointInfo {
	...
	public MD5JointInfo(List<String> blockBody) {
		List<MD5JointData> joints = new ArrayList<>();
		for (String line : blockBody) {
			MD5JointData jointData = new MD5JointData(line);
			if (jointData != null) {
	...
	public static class MD5JointData {
		...
		public MD5JointData(String line) {
			...
			if (matcher.matches()) {
				...
				this.parentIndex = Integer.parseInt(matcher.group(2));
				...
				this.orientation = MD5Utils.calculateQuaternion(new Vector3f(x, y, z));
			}
		}
	}
